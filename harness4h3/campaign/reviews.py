...
def _review_prompt(role: str, request: Mapping[str, Any]) -> str:
    ...
    if role == "advocate":
        ...
        instruction = (
            ...
            "This is a pre-training review: missing measurements that this campaign is explicitly "
            "about to collect (latency, quality, memory, target-device evidence, or convergence) "
            "are expected missing_evidence_ids, not a hard objection by themselves. Keep "
            "required_revisions empty for measurement-only gaps. Set hard_objection true, and "
            "require a revision, only for an immediately unexecutable action, an unsupported "
            "algorithm/value, an integrity violation, or a direct hard-constraint conflict. "
            "The configured Fidelity schedule is authoritative: F1 intentionally uses the "
            "small initial budget (4 steps in this campaign), and later F2/F3 add budget. Do "
            "not require target_steps >= 16 or treat an F1 budget as algorithmically invalid; "
            "convergence and quality are verifier outcomes to measure. Do not make an "
            "untested architectural hypothesis a hard objection. "
        ...
        instruction = (
            ...
            "Use only executable Student values: training.method is dmd2 or "
            "progressive_distillation; deployment.precision is bf16 or fp16; "
            "deployment.quantization is none or int8. Never invent fp8, int4, or another "
            "unregistered value. "
