...
The remote configuration must point at a reachable Ollama-compatible `/api/chat` endpoint. If Ollama is unavailable, the supervisor records `proposal_invalid`/`campaign_error` and stops; it does not substitute a rule-based architecture. The H3 example uses the real cache geometry `5×16×16`, and the target dimensions in the YAML must match the cache used by the teacher adapter.
