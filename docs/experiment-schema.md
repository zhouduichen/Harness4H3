...
Required fields are `experiment_id`, `session_id`, `target_profile_id`, controller provider/model, parent and optional child model/system IDs, ModelState and SystemState digests, diagnosis, full ExperimentPlan, operator execution result, training-log/artifact paths, measured cost, canonical `EvaluationRecord`, stable failure type, fixed continuation decision, Pareto update and UTC creation time.

Model files are not embedded. A successful model modification produces
`parent_model_id → experiment_id → child_model_id` and a paired system record;
a runtime-only modification produces `parent_system_id → experiment_id →
child_system_id` while retaining the same `model_ref`. Failed validation or
execution has no child ID and leaves the active pointers unchanged. Candidate
files are created exclusively and cannot be overwritten; `active.json`,
`front.json` and `session.json` use atomic replacement.
